  ...
  const generatePDF = (timetable) => {
    ...
    timetable.schedule.forEach((day) => {
      day.lectures.forEach((lecture, index) => {
        tableData.push([
          index === 0 ? day.day : "", // ✅ Only show the day name in the first row for that day
          lecture.start_time,
          lecture.end_time,
          lecture.module_code,
          lecture.lecturer_id,
          ...
          {selectedTimetable.schedule.map((day) => {
            return day.lectures.map((lecture, index) => (
              <tr key={index} className="border">
                ...
                {index === 0 ? (
                  <td className="p-2 font-bold" rowSpan={day.lectures.length}>
                    {day.day}
                  </td>
                ) : null}
